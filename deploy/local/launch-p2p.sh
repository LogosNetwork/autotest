...
then
  ...
  cd $exe_consensus
  rm -f logos_core
  (cmake -DBOOST_ROOT="$HOME/Documents/Projects/Tools/boost_1_67_0" -DACTIVE_NETWORK="logos_test_network" -DCMAKE_BUILD_TYPE="Debug" \
      -G "Unix Makefiles" ..\
...
if [ "$templ" = "1" ]
then
  ./maketempl.sh --instances $inst $txa $p2p
fi
